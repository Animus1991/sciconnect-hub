      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Dialog open={showNewForm} onOpenChange={setShowNewForm}>
          <DialogTrigger asChild>
            <Button className="h-9 px-4 rounded-xl gap-2 text-[13px] font-medium"><Plus className="w-4 h-4" /> Track Conference</Button>
          </DialogTrigger>
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] text-foreground truncate">{conf.name}</p>
                  <div className="flex items-center gap-4 mt-2 text-[11px] text-muted-foreground flex-wrap">
                    <span className="flex items-center gap-1 min-w-0 truncate"><MapPin className="w-3 h-3" />{conf.location}</span>
                    <span className="flex items-center gap-1 min-w-0 whitespace-nowrap"><Calendar className="w-3 h-3" />{conf.startDate} → {conf.endDate}</span>
                    <span className="flex items-center gap-1 min-w-0 whitespace-nowrap"><Users className="w-3 h-3" />{conf.attendees.toLocaleString()} attendees</span>
                  ...
                  {conf.submissions.length > 0 && (
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {conf.submissions.map(sub => {
                        ...
                        return (
                          <Badge key={sub.id} variant="outline" className={`text-[11px] px-2 py-0.5 rounded-full ${sm.color}`}>
                ...
                <div className="text-right flex-shrink-0">
                  {days > 0 ? (
                    <>
                      <p className="text-[22px] font-bold text-foreground">{days}</p>
                      <p className="text-[11px] text-muted-foreground">days away</p>
                    </>
                  ) : (
                    <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 rounded-full px-2 py-0.5 text-[11px]">Happening now</Badge>
                  )}
                  {nextDeadline && (
                    <div className="mt-2">
                      <p className="text-[11px] text-muted-foreground">Next: {nextDeadline.label}</p>
                      <p className="text-[13px] font-medium text-foreground">{nextDeadline.date}</p>
                    </div>
                  )}
                </div>
              </div>
              {conf.tags.length > 0 && (
                <div className="flex gap-1.5 mt-3 flex-wrap">
                  {conf.tags.map(t => <Badge key={t} variant="secondary" className="text-[11px] px-2 py-0.5 rounded-full">{t}</Badge>)}
                </div>
              )}
            </motion.div>
          );
