          ...
          <TabsContent value="browse">
            <div className="relative mb-6">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                ...
                className="w-full h-10 pl-10 pr-4 rounded-xl bg-card border border-border text-[13px] font-display placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredCourses.length === 0 ? (
                <div className="col-span-full text-center py-12 bg-card rounded-xl border border-border">
                  ...
                  <p className="text-[13px] text-muted-foreground font-display">No courses match your search</p>
                </div>
              ) : filteredCourses.map((course, i) => (
                <motion.div key={course.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                  className="bg-card rounded-xl border border-border p-5 hover:border-accent/30 hover:shadow-scholarly transition-all cursor-pointer group flex flex-col">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2 flex-wrap min-w-0">
                      <Badge variant="outline" className={`rounded-full text-[11px] px-2 py-0.5 font-medium font-display ${levelStyles[course.level]}`}>{course.level}</Badge>
                      {course.featured && (
                        <Badge variant="outline" className="rounded-full text-[11px] px-2 py-0.5 font-medium font-display text-gold border-gold/30 bg-gold-muted">Featured</Badge>
                      )}
                    </div>
                    <div className="flex items-center gap-1 text-[12px] text-muted-foreground font-display flex-shrink-0">
                      <Star className="w-3.5 h-3.5 text-gold fill-gold" /> {course.rating}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {course.tags.map(tag => <Badge key={tag} variant="secondary" className="rounded-full text-[11px] px-2 py-0.5 font-medium font-display">{tag}</Badge>)}
                  </div>

                  <div className="flex items-center gap-3 text-[12px] text-muted-foreground font-display mb-4">
                    <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {course.duration}</span>
                    <span className="flex items-center gap-1"><BarChart3 className="w-3.5 h-3.5" /> {course.modules} modules</span>
                  </div>

                  {enrolled.has(course.id) && course.progress > 0 && (
                    <div className="flex items-center gap-3 mb-4">
                      <Progress value={course.progress} className="h-1.5 rounded-full bg-secondary flex-1 [&>div]:bg-primary" />
                      <span className="text-[12px] text-muted-foreground font-display font-medium">{course.progress}%</span>
                    </div>
                  )}

                  <div className="mt-auto">
                    <button onClick={(e) => { e.stopPropagation(); toggleEnroll(course.id, course.title); }}
                      className={`w-full h-9 rounded-xl text-[13px] font-medium transition-all flex items-center justify-center gap-1.5 ${
                        enrolled.has(course.id)
                          ? "bg-success/10 text-success hover:bg-success/20"
                          : "bg-accent text-accent-foreground hover:opacity-90"
                      }`}>
                      {enrolled.has(course.id)
                        ? (course.progress > 0 ? <><Play className="w-4 h-4" /> Continue</> : <><CheckCircle2 className="w-4 h-4" /> Enrolled</>)
                        : "Enroll Now"
                      }
                    </button>
                  </div>
