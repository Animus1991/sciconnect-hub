  ...
  >
    ...
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-semibold text-foreground text-[16px]">NDA Mode Activation</h3>
          <p className="text-[11px] text-muted-foreground">{convName}</p>
        </div>
      </div>
        ...
        <ul className="space-y-1.5">
          {[
            ...
            <li key={i} className="flex items-start gap-2 text-[12px] text-muted-foreground">
              <CheckSquare className="w-3 h-3 text-primary flex-shrink-0 mt-0.5" />
              {item}
            </li>
          ))}
        </ul>
      ...
      <div className="flex gap-2">
        <Button variant="outline" className="flex-1 text-[13px] font-medium h-10 rounded-xl" onClick={onDecline}>
          Decline
        </Button>
        <Button className="flex-1 text-[13px] font-medium h-10 rounded-xl bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={onAccept}>
          Accept NDA Terms
        </Button>
      </div>
    </motion.div>
...
const ChatTypingIndicator = ({ name }: { name: string }) => (
  ...
  >
    ...
    <div className="bg-card border border-border rounded-xl rounded-bl-md px-3 py-2 flex items-center gap-1">
      {[0, 1, 2].map(i => (
        <motion.span
          key={i}
...
const Messenger = () => {
  ...
  return (
    <AppLayout>
      <TooltipProvider>
        <div className="flex h-[calc(100vh-56px)] overflow-hidden bg-card relative">
            ...
            <div className="flex-1 flex flex-col min-w-0">
              {activeConv ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
                  <div className="w-20 h-20 rounded-xl bg-secondary/30 flex items-center justify-center mb-5">
                    <MessageCircle className="w-10 h-10 text-muted-foreground/20" />
                  </div>
                  <h2 className="font-semibold text-[15px] text-foreground mb-1.5">Research Messenger</h2>
                  <p className="text-[13px] text-muted-foreground font-display max-w-[320px] leading-relaxed">
