    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[15px] font-semibold text-foreground">Live Activity</h3>
          {newCount > 0 && (
        ...
        <AnimatePresence mode="popLayout">
          {activities.map((a) => (
            <motion.div
              ...
              className={`flex items-center h-10 gap-2.5 p-2 rounded-lg transition-colors ${a.isNew ? "bg-accent/5 border border-accent/10" : "hover:bg-secondary/40"}`}
            >
              <Avatar className="w-7 h-7 shrink-0">
                <AvatarFallback className="bg-scholarly text-primary-foreground text-[10px] font-bold">
                  {a.initials}
                </AvatarFallback>
              </Avatar>
              <div className="min-w-0 flex-1 flex flex-col justify-center">
                <p className="text-[12px] text-foreground leading-tight truncate">
                  <span className="font-semibold">{a.name}</span>{" "}
                  <span className="text-muted-foreground">{a.action}</span>{" "}
                  <span className="text-accent">{a.emoji} {a.target}</span>
                </p>
              ...
              {a.isNew && <Zap className="w-3 h-3 text-warning shrink-0" />}
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
