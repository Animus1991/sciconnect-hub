          ...
          <TabsContent value="all" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.length === 0 ? (
              <div className="col-span-full text-center py-12 bg-card rounded-xl border border-border">
                ...
                <p className="text-[13px] text-muted-foreground font-display">No opportunities match your search</p>
              </div>
            ) : filtered.map((opp, i) => {
                ...
                <motion.div key={opp.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2 flex-wrap min-w-0">
                      <Badge variant="outline" className={`text-[11px] font-display font-medium ${cfg.cls}`}>
                        <TypeIcon className="w-3 h-3 mr-1" />{cfg.label}
                      </Badge>
                      {opp.featured && (
                        <Badge variant="outline" className="text-[11px] font-display text-gold border-gold/30 bg-gold-muted font-medium">Featured</Badge>
                      )}
                      {opp.daysLeft <= 30 && (
                        <Badge variant="outline" className="text-[11px] font-display text-warning border-warning/30 bg-warning/10 font-medium">
                          <Clock className="w-3 h-3 mr-1" />{opp.daysLeft}d left
                        </Badge>
                      )}
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); toggleSave(opp.id, opp.title); }}
                      className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-secondary transition-colors flex-shrink-0 ml-2"
                  ...
                  <div className="flex flex-col gap-1.5 text-[13px] text-muted-foreground font-display mb-4">
                    <span className="flex items-center gap-1.5"><Building2 className="w-3.5 h-3.5" /> <span className="truncate">{opp.organization}</span></span>
                    <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> <span className="truncate">{opp.location}</span></span>
                    <span className="flex items-center gap-1.5"><DollarSign className="w-3.5 h-3.5" /> {opp.funding}</span>
                    <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {opp.deadline}</span>
                  </div>
                  
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {opp.tags.map(tag => <Badge key={tag} variant="secondary" className="rounded-full text-[11px] px-2 py-0.5 font-medium">{tag}</Badge>)}
                  </div>

                  <div className="mt-auto flex gap-2">
                    <button
                      className="flex-1 h-9 px-4 rounded-xl bg-accent text-accent-foreground text-[13px] font-medium font-display transition-opacity hover:opacity-90"
                    >
                      Apply Now
                    </button>
                  </div>
