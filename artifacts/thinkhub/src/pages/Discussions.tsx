        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between mb-6">
              <div>
                <h1 className="font-serif text-[27px] font-bold text-foreground mb-1">Discussions</h1>
                ...
                >
                  <div className="bg-card rounded-xl border border-border p-4 sm:p-5 space-y-4">
                    ...
                    <input
                      ...
                      className="w-full h-10 px-4 rounded-lg bg-secondary/50 border border-border text-sm font-display placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent"
                    />
                    <div className="flex gap-2 flex-wrap">
                      {(Object.entries(categoryConfig) as [Category, typeof categoryConfig[Category]][]).map(([key, cfg]) => (
                        <button
                          key={key}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-display transition-all border border-transparent hover:border-border ${cfg.color}`}
                        >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-1.5 h-10 px-3 rounded-lg bg-card border border-border text-xs font-display text-muted-foreground hover:text-foreground transition-colors"
                  >
                  ...
                  <AnimatePresence>
                    {showSortMenu && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -4 }}
                        className="absolute right-0 top-12 z-20 bg-popover border border-border rounded-lg shadow-scholarly p-1 min-w-[160px]"
                      >
                        {sortOptions.map(opt => (
                          <button
                            key={opt.key}
              ...
              <div className="flex items-center gap-1 overflow-x-auto scrollbar-thin pb-1">
                {tabs.map(tab => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-display font-medium whitespace-nowrap transition-all ${
                      activeTab === tab.key
                    ...
                    >
                      <div className="flex items-start gap-2 sm:gap-3">
                        ...
                        <div className="flex flex-col items-center gap-0.5 pt-0.5 min-w-[36px] sm:min-w-[40px]">
                          ...
                          >
                            <ArrowUp className={`w-3.5 h-3.5 ${isLiked ? "fill-current" : ""}`} />
                            <span className="text-[10px] font-mono font-semibold">{d.likes + (isLiked ? 1 : 0)}</span>
                          </button>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                            ...
                            {d.solved && (
                              <Tooltip><TooltipTrigger><CheckCircle2 className="w-3.5 h-3.5 text-emerald-brand flex-shrink-0" /></TooltipTrigger>
                                <TooltipContent><p className="text-xs">Solved</p></TooltipContent></Tooltip>
                            )}
                            {isHot && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded flex items-center gap-0.5 font-display font-bold text-warning bg-warning-muted">
                                <Flame className="w-2.5 h-2.5" /> HOT
                              </span>
                            )}
                            <h3 className="font-display font-semibold text-sm text-foreground group-hover:text-accent transition-colors line-clamp-1 flex-1">
                              {d.title}
                            </h3>
