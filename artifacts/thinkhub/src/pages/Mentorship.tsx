            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredMentors.length === 0 ? (
                <div className="col-span-full text-center py-12 bg-card rounded-xl border border-border">
                  ...
                  <p className="text-[13px] text-muted-foreground font-display">No mentors match your search</p>
                </div>
                ...
                >
                  <div className="flex items-start gap-4 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-display font-semibold text-[15px] text-foreground group-hover:text-accent transition-colors line-clamp-1">
                        {mentor.name}
                      </h3>
                      <p className="text-[12px] text-muted-foreground font-display mb-1 line-clamp-1">{mentor.title}</p>
                      <p className="text-[12px] text-muted-foreground font-display flex items-center gap-1.5 line-clamp-1">
                        <Building2 className="w-3.5 h-3.5 flex-shrink-0" /> <span className="truncate">{mentor.institution}</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {mentor.fields.map((field) => (
                      <Badge key={field} variant="secondary" className="rounded-full text-[11px] px-2 py-0.5 font-medium">{field}</Badge>
                    ))}
                  </div>

                  <div className="flex items-center justify-between mt-auto mb-4">
                    <div className="flex items-center gap-3 text-[12px] text-muted-foreground font-display">
                      <span className="flex items-center gap-1 font-medium text-foreground">
                        <Star className="w-3.5 h-3.5 text-gold fill-gold" /> {mentor.rating}
                      </span>
                      <span className="flex items-center gap-1">
                        <Users className="w-3.5 h-3.5" /> {mentor.mentees} mentees
                      </span>
                    </div>
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); toggleRequest(mentor.name); }}
                    className={`w-full h-9 px-4 rounded-xl text-[13px] font-display font-medium transition-all flex items-center justify-center gap-1.5 ${
                      requested.has(mentor.name)
                        ? "bg-success/10 text-success"
                        : "bg-accent text-accent-foreground hover:opacity-90"
                    }`}
                  >
                    {requested.has(mentor.name) ? <><Check className="w-4 h-4" /> Requested</> : "Request Connection"}
                  </button>
                </motion.div>
              ))}
            </div>
