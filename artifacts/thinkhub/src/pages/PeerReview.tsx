        ...
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <div className="flex items-start justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => setShowSubmitForm(true)} className="font-display gap-1.5 h-10 px-5 rounded-xl">
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-3 flex-wrap">
                          <div className={`flex items-center gap-1 text-[11px] font-display font-medium px-2 py-0.5 rounded-full ${config.bg} ${config.color}`}>
                            <config.icon className="w-3 h-3" />
                            {config.label}
                          </div>
                          <span className="text-[11px] text-muted-foreground font-mono">#{review.id}</span>
                          {review.priority === "high" && (
                            <Badge variant="destructive" className="text-[11px] font-display font-medium rounded-full px-2 py-0.5">High</Badge>
                          )}
                        </div>
                        <button onClick={() => toggleExpand(review.id)} className="text-left w-full mb-3">
                          <h3 className="font-display text-[14px] font-semibold text-foreground leading-snug group-hover:text-accent transition-colors line-clamp-2">
                            {review.title}
                          </h3>
                        </button>
                        <div className="flex items-center gap-3 text-[12px] text-muted-foreground font-display mt-2 mb-4 flex-wrap">
                          <span className="flex items-center gap-1"><BookOpen className="w-3.5 h-3.5" /> {review.journal}</span>
                          <span className="flex items-center gap-1"><Microscope className="w-3.5 h-3.5" /> {review.field}</span>
                        ...
                        {review.progress !== undefined && (
                          <div className="mt-3 mb-4 flex items-center gap-3">
                            <Progress value={review.progress} className="h-1.5 rounded-full bg-secondary flex-1 [&>div]:bg-primary" />
                            <span className="text-[12px] text-muted-foreground font-display font-medium">{review.progress}%</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-auto pt-2">
                        <button onClick={() => cycleStatus(review.id)}
                          className={`flex-1 h-8 text-[13px] font-display font-medium px-3 rounded-xl transition-all ${
                        ...
                        >
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <button className="w-8 h-8 rounded-xl bg-secondary flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-secondary/80 transition-colors">
                          ...
                          <button onClick={() => toggleExpand(review.id)} className="w-8 h-8 rounded-xl bg-secondary flex items-center justify-center hover:bg-secondary/80 transition-colors">
                        ...
                        {isExpanded && (
                          ...
                          >
                            <div className="mt-4 pt-4 border-t border-border space-y-3">
                              {review.authors && (
                                <div>
                                  <span className="text-[11px] font-display text-muted-foreground uppercase tracking-wider font-medium">Authors</span>
                                  <p className="text-[13px] font-display text-foreground mt-0.5">{review.authors}</p>
                                </div>
                              )}
                              {review.abstract && (
                                <div>
                                  <span className="text-[11px] font-display text-muted-foreground uppercase tracking-wider font-medium">Abstract</span>
                                  <p className="text-[13px] font-display text-muted-foreground leading-relaxed mt-0.5">{review.abstract}</p>
                                </div>
                              )}
                              <div className="flex items-center gap-3">
                                <span className="text-[11px] font-display text-muted-foreground uppercase tracking-wider font-medium">Deadline</span>
                                <span className="text-[13px] font-display text-foreground">{review.deadline}</span>
                              </div>
                              {review.progress !== undefined && (
                                <div className="flex gap-2">
                                  <Button size="sm" variant="outline" onClick={() => updateProgress(review.id, -10)} className="font-display text-[12px] h-8 rounded-lg px-3">−10%</Button>
                                  <Button size="sm" variant="outline" onClick={() => updateProgress(review.id, 10)} className="font-display text-[12px] h-8 rounded-lg px-3">+10%</Button>
                                  <Button size="sm" variant="outline" onClick={() => updateProgress(review.id, 100 - (review.progress ?? 0))} className="font-display text-[12px] h-8 rounded-lg px-3">Complete</Button>
                                </div>
                              )}
                              <div className="pt-2">
                                <Button size="sm" onClick={() => { setShowSubmitForm(true); setExpandedId(null); }} className="h-9 px-4 rounded-xl text-[13px] font-medium w-full">
