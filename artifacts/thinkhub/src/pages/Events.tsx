          ...
          {typeFilters.map(type => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl border text-[13px] font-display font-medium transition-all capitalize ${
                  typeFilter === type
          ...
          <TabsContent value="all" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.length === 0 ? (
              <div className="col-span-full text-center py-12 bg-card rounded-xl border border-border">
                ...
                <p className="text-[13px] text-muted-foreground font-display">No events match your search</p>
              </div>
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  <Badge variant="outline" className={`h-7 rounded-lg px-2.5 bg-primary/10 text-primary border-0 text-[12px] font-display font-medium`}>
                    <Calendar className="w-3.5 h-3.5 mr-1" /> {event.date}
                  </Badge>
                  {event.attending && (
                     <Badge variant="outline" className="text-[11px] font-display text-success border-success/20 bg-success/10 font-medium">
                      Attending
                    </Badge>
                  )}
                </div>

                <div className="flex flex-col gap-1.5 text-[12px] text-muted-foreground font-display mb-4">
                  <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {event.location}</span>
                  <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" /> {event.attendees.toLocaleString()} attendees</span>
                  {event.deadline && (
                    <span className={`flex items-center gap-1.5 ${event.daysToDeadline! < 21 ? "text-warning" : ""}`}>
                      <Clock className="w-3.5 h-3.5" /> Deadline: {event.deadline}
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {event.tags.map((tag) => (
                    <Badge key={tag} variant="secondary" className="rounded-full text-[11px] px-2 py-0.5 font-medium">{tag}</Badge>
                  ))}
                </div>

                <div className="mt-auto">
                  <button
                    onClick={(e) => { e.stopPropagation(); toggleAttending(event.title); }}
                    className={`w-full text-[13px] font-display font-medium h-9 px-4 rounded-xl transition-all ${
                      event.attending
