      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full h-11 pl-10 pr-4 rounded-lg bg-secondary/30 border border-border text-sm font-display text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent transition-all"
              />
            </div>
            <button onClick={handleSearch} disabled={searching || !query.trim()}
              className="h-11 px-6 rounded-lg gradient-gold text-accent-foreground font-display font-semibold text-sm flex items-center gap-2 shadow-gold hover:opacity-90 transition-opacity disabled:opacity-50">
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <span className="text-[10px] text-muted-foreground font-display uppercase tracking-wider">Searching in:</span>
            {Object.entries(SOURCE_META).map(([key, meta]) => (
              <span key={key} className="inline-flex items-center gap-1 text-xs font-display bg-secondary/50 rounded-full px-2.5 py-0.5">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-muted-foreground font-display">Sort:</span>
                <select value={sortBy} onChange={e => setSortBy(e.target.value as typeof sortBy)}
                  className="text-xs font-display bg-card border border-border rounded-lg px-2.5 py-1.5 text-foreground focus:outline-none focus:ring-2 focus:ring-accent">
            ...
            {searching && (
              <div className="flex flex-col items-center justify-center py-16 bg-card rounded-xl border border-border">
                ...
                <p className="text-sm text-muted-foreground font-display">Searching across repositories...</p>
                <div className="flex gap-3 mt-3">
                  {Object.entries(SOURCE_META).map(([key, meta]) => (
                    <span key={key} className="inline-flex items-center gap-1 text-xs text-muted-foreground font-display animate-pulse">
                      ...
                      <motion.div key={result.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <div className="flex-1 min-w-0">
                            ...
                            <h3 className="font-display font-semibold text-foreground text-sm leading-snug">{result.title}</h3>
                          </div>
                        ...
                        <div className="flex items-center gap-1.5 mb-2">
                          ...
                          <p className="text-xs text-muted-foreground font-display truncate">{result.authors.join(", ")}</p>
                        </div>

                        <div className={`text-xs text-muted-foreground font-display leading-relaxed mb-3 ${expandedAbstract === result.id ? "" : "line-clamp-2"}`}>
                          {result.abstract}
                        </div>
                        {result.abstract.length > 100 && (
                          <button onClick={() => setExpandedAbstract(expandedAbstract === result.id ? null : result.id)}
                            className="text-[10px] text-accent font-display font-medium mb-3 flex items-center gap-0.5">
        ...
        {!hasSearched && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
            ...
            <h2 className="font-serif text-lg font-bold text-foreground mb-2">Search Scientific Literature</h2>
            <p className="text-sm text-muted-foreground font-display max-w-md mx-auto mb-6">
            ...
            <div className="flex justify-center gap-4">
              {["Quantum Computing", "CRISPR", "Transformer Models"].map(suggestion => (
                <button key={suggestion} onClick={() => { setQuery(suggestion); }}
                  className="text-xs font-display bg-secondary/50 hover:bg-secondary rounded-full px-4 py-2 text-foreground transition-colors">
                  {suggestion}
                </button>
              ))}
            </div>
