    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-xl bg-accent/10 flex items-center justify-center">
            <Brain className="w-3.5 h-3.5 text-accent" />
          </div>
          <h3 className="text-[15px] font-semibold text-foreground">Recommended for You</h3>
          ...
          {visibleRecs.slice(0, 4).map((rec, i) => {
            ...
            return (
              <motion.div
                ...
                className={`p-4 rounded-xl border transition-all ${rec.isNew ? "border-accent/20 bg-accent/5" : "border-transparent bg-secondary/30 hover:bg-secondary/50"}`}
              >
                <div className="flex items-start gap-3">
                  <span className={`text-sm mt-0.5 w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${style.color}`}>
                    {style.emoji}
                  </span>
