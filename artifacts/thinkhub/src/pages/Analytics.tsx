          ...
          <div className="flex items-start justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleExportReport}
                className="h-8 px-3 rounded-lg text-[13px] font-display font-medium flex items-center gap-1.5 border bg-card border-border text-muted-foreground hover:text-foreground transition-colors">
                <Download className="w-3.5 h-3.5" /> Export
              </button>
              <button onClick={() => setComparisonMode(!comparisonMode)}
                className={`h-8 px-3 rounded-lg text-[13px] font-display font-medium flex items-center gap-1.5 border transition-colors ${
              ...
              <div className="flex items-center bg-card border border-border rounded-lg p-0.5 gap-2">
                {dateRangeOptions.map(opt => (
                  <button key={opt.value} onClick={() => setDateRange(opt.value)}
                    className={`px-2.5 py-1.5 rounded-md text-[13px] font-display font-medium transition-all ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5">
          ...
          <div className="space-y-5">
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              ...
              {/* Tab strip */}
              <div className="flex items-center gap-2 px-4 pt-4 pb-2 border-b border-border/50 overflow-x-auto">
                {chartTabs.map(tab => (
                  <button key={tab.value} onClick={() => setChartView(tab.value)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-display font-medium transition-all whitespace-nowrap ${
                      chartView === tab.value
