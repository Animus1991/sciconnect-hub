      ...
      <div className="flex gap-6 max-w-7xl">
        ...
        <div className="flex-1 min-w-0">
          ...
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between mb-6">
            <div>
              <h1 className="font-serif text-[1.78rem] font-bold text-foreground mb-1">Research Groups</h1>
            ...
            <button
              ...
              className="h-9 px-4 rounded-lg gradient-gold text-accent-foreground font-display font-semibold text-sm flex items-center gap-2 shadow-gold hover:opacity-90 transition-opacity"
            >
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="grid grid-cols-4 gap-3 mb-5">
            {[
              ...
              <div key={s.label} className="bg-card rounded-xl border border-border p-3.5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-lg font-display font-bold text-foreground leading-tight">{s.value}</p>
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full h-9 pl-9 pr-4 rounded-lg bg-card border border-border text-sm font-display placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent"
                />
              </div>
              ...
              <div className="flex items-center gap-1">
                {(["all", "public", "private"] as const).map(ft => (
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-lg text-[11px] font-display font-semibold transition-all flex items-center gap-1 ${
                      filterType === ft
              ...
              <div className="relative">
                <select
                  ...
                  className="h-9 pl-7 pr-3 rounded-lg bg-card border border-border text-[11px] font-display font-semibold text-muted-foreground appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-accent"
                >
            ...
            {filteredGroups.map((group, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-card rounded-xl border border-border hover:border-accent/20 transition-all"
                >
                  {/* Card Header */}
                  <div
                  ...
                  >
                    <div className="flex items-start gap-3.5">
                      <div className={`w-11 h-11 rounded-xl ${group.color} flex items-center justify-center text-white font-display font-bold text-sm flex-shrink-0`}>
                        {group.initials}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          {isMember && (
                            <Badge variant="secondary" className="text-[9px] h-4 px-1.5 font-display">Member</Badge>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground font-display">
                          ...
                          <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5" /> {group.activeToday} active today</span>
                          <div className="ml-auto flex items-center gap-1">
                            ...
                            <button
                              ...
                              className={`text-xs font-display font-semibold px-2.5 py-1 rounded-lg transition-all flex items-center gap-1 ${
                                isMember
                                  ? "bg-success/10 text-success"
                                  : "bg-accent/10 text-accent hover:bg-accent/20"
                              }`}
                            >
                              {isMember ? <><Check className="w-3 h-3" /> Joined</> : <><UserPlus className="w-3 h-3" /> Join</>}
                            </button>
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div className="px-4 pb-4 border-t border-border pt-3 space-y-3">
                          ...
                          {/* Pinned post */}
                          <div className="flex items-start gap-2 bg-gold-muted rounded-lg p-3">
                          ...
                          <div className="flex items-center gap-2 pt-1">
                            <button
                              ...
                              className="h-8 px-3 rounded-lg bg-accent text-accent-foreground text-xs font-display font-semibold flex items-center gap-1.5 hover:opacity-90 transition-opacity"
                            >
                            ...
                            {isMember && (
                              <button
                                ...
                                className={`h-8 px-3 rounded-lg border border-border text-xs font-display font-semibold flex items-center gap-1.5 transition-colors ${
            ...
            <div className="space-y-2">
              {[...groups].sort((a, b) => b.members - a.members).slice(0, 4).map((g, idx) => (
                <div key={g.name} className="flex items-center gap-2.5">
                  ...
                  <div className={`w-7 h-7 rounded-lg ${g.color} flex items-center justify-center text-white text-[10px] font-display font-bold flex-shrink-0`}>
                    {g.initials}
                  </div>
                  <div className="flex-1 min-w-0">
