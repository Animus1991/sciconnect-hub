  ...
  return (
    <div className="h-14 px-3 sm:px-4 flex items-center justify-between border-b border-border bg-card flex-shrink-0">
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div onClick={onToggleInfo} className="flex items-center gap-2.5 min-w-0 hover:opacity-80 transition-opacity cursor-pointer">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <h2 className="text-[14px] font-semibold text-foreground truncate">{conversation.name}</h2>
              {bcLevel !== "off" && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="cursor-help"><BlockchainIcon className={`w-3.5 h-3.5 flex-shrink-0 ${bcColor}`} /></div>
                  </TooltipTrigger>
              ...
              {isNDA && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="cursor-help"><Lock className="w-3 h-3 text-destructive flex-shrink-0" /></div>
                  </TooltipTrigger>
      ...
      <div className="flex items-center gap-0.5">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64">
                <div className="px-3 py-2 border-b border-border">
                  <p className="text-[12px] font-display font-semibold text-foreground">IP Verification Level</p>
                ...
                {blockchainLevels.map(b => (
                  <DropdownMenuItem
                    ...
                    className={`text-[13px] gap-2 ${bcLevel === b.level ? "bg-secondary" : ""}`}
                  >
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-52">
            {isMobile && (
              <>
                <DropdownMenuItem className="text-[13px] gap-2" onClick={() => onStartCall("voice")}>
                  <Phone className="w-3.5 h-3.5" /> Voice call
                </DropdownMenuItem>
                <DropdownMenuItem className="text-[13px] gap-2" onClick={() => onStartCall("video")}>
            ...
            <DropdownMenuItem className="text-[13px] gap-2" onClick={onToggleNDA}>
              {isNDA ? <Unlock className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
              {isNDA ? "Disable NDA Mode" : "Enable NDA Mode"}
            </DropdownMenuItem>
            <DropdownMenuItem className="text-[13px] gap-2" onClick={onExportLabRecord}>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="text-[13px] gap-2 text-destructive">
