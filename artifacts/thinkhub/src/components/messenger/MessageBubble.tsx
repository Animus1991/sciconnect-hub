    ...
    return (
      <div className={`flex ${isMine ? "justify-end" : "justify-start"} mb-1`}>
        <div className="px-3 py-2 rounded-2xl bg-secondary/20 border border-border/20 max-w-[75%]">
          <p className="text-[12px] text-muted-foreground/50 font-display italic flex items-center gap-1.5">
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[70%] sm:max-w-[65%]">
        ...
        {/* Bubble */}
        <div className={`rounded-2xl px-4 py-2.5 max-w-[70%] text-[13px] relative ${
          isMine
                ...
                <div key={i}>
                  ...
                  {att.type === "file" && (
                    <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg cursor-pointer hover:opacity-80 transition-opacity ${
                      ...
                      <div className="min-w-0">
                        <p className={`text-[12px] font-display font-medium truncate ${isMine ? "text-accent-foreground" : "text-foreground"}`}>{att.name}</p>
                  ...
                  {att.type === "location" && (
                    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${
                      ...
                      <span className={`text-[12px] font-display ${isMine ? "text-accent-foreground" : "text-foreground"}`}>{att.name}</span>
                    </div>
                  )}
                </div>
              ))}
          ...
          <div className={`text-[13px] leading-relaxed ${isMine ? "text-primary-foreground" : "text-foreground"}`}>
            <RichMessageContent text={msg.text} isMine={isMine} />
          </div>

          {/* Time + Status + Blockchain */}
          <div className={`flex items-center gap-1 mt-1 ${isMine ? "justify-end" : ""}`}>
            {msg.edited && <span className={`text-[9px] italic ${isMine ? "text-primary-foreground/40" : "text-muted-foreground/50"}`}>edited</span>}
            {msg.blockchainHash && (
              <span className={`text-[8px] font-mono ${isMine ? "text-primary-foreground/40" : "text-muted-foreground/40"} flex items-center gap-0.5`}>
                <Shield className="w-2 h-2" />{msg.blockchainHash}
              </span>
            )}
            <span className={`text-[10px] tabular-nums ${isMine ? "text-primary-foreground/60" : "text-muted-foreground/60"}`}>{msg.time}</span>
            {isMine && (
              <span className="flex items-center ml-0.5">
                {msg.status === "sending" && <Circle className="w-2.5 h-2.5 text-primary-foreground/30" />}
                ...
                {msg.status === "read" && <CheckCheck className="w-3 h-3 text-primary-foreground/90" />}
              </span>
            )}
          </div>
        </div>
            ...
            >
              {evidenceTypes.map(et => (
                ...
                >
                  ...
                  <span className="text-[10px] font-display text-muted-foreground whitespace-nowrap">{et.label}</span>
                </button>
              ))}
            </motion.div>
          )}
